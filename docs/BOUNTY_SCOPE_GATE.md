...
The ProtocolGate MCP server exposes:

```text
protocolgate_bounty_reportability_gate
```
...
Use it before report writing. Do not draft a bounty submission until the gate
returns `submit`, or the `defer` evidence gaps are resolved.

## ProtocolGate Sales Angle

This is also buyer-facing.

The value is not "we found every bug." The value is:

> We can tell your team which scary control-plane reports are real, which are
> missing evidence, and which will turn into bounty triage noise.

That is useful before:
