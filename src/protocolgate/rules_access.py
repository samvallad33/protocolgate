...
def require_governance_floor(manifest: Manifest) -> Iterable[Violation]:
    ...
    if not governance:
        return
    if as_int(governance.get("quorum_bps"), field="governance.quorum_bps") < 400:
        yield Violation(
            "CG024",
            "medium",
    ...
    if as_int(governance.get("voting_period_seconds"), field="governance.voting_period_seconds") < MIN_ADMIN_TIMELOCK_SECONDS:
        yield Violation(
            "CG025",
            "medium",
