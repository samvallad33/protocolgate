...
from protocolgate.manifest import ManifestError
...
def as_int(value: Any, *, field: str, default: int = 0) -> int:
    """Coerce a manifest numeric field to ``int`` or raise ManifestError.

    Missing, ``None`` and empty values fall back to ``default`` (preserving the
    previous ``int(value or 0)`` behaviour). A non-numeric value raises
    ManifestError so the CLI reports a clean schema error (exit 2) instead of
    crashing with a traceback, and never silently coerces a security-relevant
    field such as a multisig threshold to a passing default.
    """
    if value is None or value == "":
        return default
    if isinstance(value, bool):
        raise ManifestError(f"{field} must be an integer, got {value!r}")
    try:
        return int(value)
    except (TypeError, ValueError) as exc:
        raise ManifestError(f"{field} must be an integer, got {value!r}") from exc
...
def timelock_delay(manifest: Manifest, ref: Any) -> int:
    ...
    if not item:
        return 0
    return as_int(item.get("delay_seconds"), field="timelocks[].delay_seconds")
